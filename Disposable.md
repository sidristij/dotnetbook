...
Что здесь не так и почему мы ранее никогда так не писали? На самом деле шаблон хороший и без лишних слов охватывает все жизненные ситуации. Но его использование повсеместно не является правилом хорошего тона. Мало того он нарушает принцип разделения ответственности. Ведь он одновременно управляет и управляемыми ресурсами и неуправляемыми. На мой скромный взгляд это совершенно не правильно. Давайте взглянем на несколько иной подход. *Disposable Design Principle*. Если коротко, то суть в следующем:

Disposing разделяется на два уровня классов:

  - Типы нулевого уровня напрямую инкапсулируют неуправляемые ресурсы
    - Они являются либо абстрактными либо запакованными
    - Должны быть разработаны таким образом чтобы исполняться внутри CER регионов
      - Для Constraint Execution Regions это означает что типы уровня 0 должны наследовать семейство типов SafeHandle (который в свою очередь наследует CriticalFinalizerObject)
      - Для *finally* блоков это означает что 
