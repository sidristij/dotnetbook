...
Вызов CheckDisposed() необходимо вызывать первым выражением во всех публичных методах класса. Однако, если для управляемого ресурса, коим является ```DisposableResource``` это выглядит нормально, то для неуправляемого ресурса - нет.
