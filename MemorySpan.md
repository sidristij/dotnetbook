...
Первое что сильно бросается в глаза - это даже не то, что эта структура не является `ref` структурой, а наличие метода `Pin()`, который, судя из названия сообщает сборщику мусора, что данный буфер при сборке мусора двигать нельзя, возвращая пользователю экземпляр структуры `MemoryHandle`, инкапсулирующей в себе понятие отрезка жизни `GCHandle`, закрепившего буфер в памяти:
