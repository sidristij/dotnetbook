...
Но в любом случае это не дольше копирования из одной переменной в другую. Почему же не выбрать ValTypes для всех типов, проектируемх в системе? Попробуем найти причины:

  1. Размещение большого количества ValTypes в стеке приведет к тому что он скоро закончится. Стек потока весьма ограничен и не может расширяться динамически;
  2. Стоит избегать больших массивов, внутри которых находятся большие структуры. Это может привести к уходу в LargeObjectsHeap и его фрагментации; 
