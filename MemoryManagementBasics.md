...
Отсутствие таблицы виртуальных методов хоть и отнимает у структур часть "магии", которую вносит понятие наследования, но и наделяет рядом преимуществ. Первое и самое главное уже было оговорено: мы можем легко и просто отдать во внешний мир (за пределы .NET Framework) экземпляр такой структуры. Это ведь просто участок памяти! Либо мы можем принять из unmanaged кода некий участок памяти и сделать приведение типа к нашей структуре чтобы сделать более удобный доступ к ее полям. С классами такое поведение не пройдет: у классов существует два поля, которые никому не доступны: это SyncBlockIndex и адрес таблицы виртуальных методов. Если эти два поля уйдут в unmanaged код, это станет очень опасным. Ведь с любой таблицы виртуальных методов можно умеючи достучаться до любого типа (VirtMethodsTable -> Type -> Domain -> AnyType) и поменять его на низком уровне, осуществив атаку на приложение.

Давайте докажем что структура - это просто участок памяти:
...
Если запускать этот код под отладчиком, то будут вставлены managed debugger assistants (MDA) вызовы. Соответственно, стек замет больше памяти и дальше идти за переменными. Если вывести значения полей `secret` и `secret2`, то мы увидем `666` и `999` соответственно. Если же мы перепишем значения более ранний RET на более поздние, мы таким образом вырежем из стека вызов метода и выход из `SecondMethod` приведет сразу к возврату в `Main`. Но это - лирическое отступление. Про эту тему мы очень подробно поговорим в разделе о стеке потока.

#### Базовый тип - Object и возможность реализации интерфейсов. Boxing.

Что интересно - так это то, что все типы обязаны наследовать тип `Object`. Иначе .NET Framework была бы совсем другой.. Но.. Если мы вспомним разговор предыдущего раздела, то заметим что ValueTypes у нас адрес таблицы виртуальных методов не содержит. А это значит что наследование от Object - это некоторая эмуляция, которая в обычных ситуациях именно эмуляция и никакого наследования нет. Т.е. в следующем коде:

```csharp
var x = 5;
SomeStructure instance;
```

Реальных объектов нет. Для чего же тогда введено наследование? Например, для таких ситуаций:
...
Т.е. когда нечто работает именно с типом Object, говоря тем самым что может быть принятно что угодно: в том числе и ValueTypes. В таких случаях происходит специальная операция, которая называется **упаковкой**. Это значит что CLR выделяет в куче место под наш ValueType, превращает его в ReferenceType (добавляя SyncBlockIndex и табл. виртуальных методов типа, экземпляр которого подвергается упаковке) и возвращает нам ссылку на экземпляр, которая в нашем случае отдается в метод, принимающий object.

С одной стороны перед нами возникает прекрасная возможность отдавать экземплары значимых типов методам, принимающих тип object.