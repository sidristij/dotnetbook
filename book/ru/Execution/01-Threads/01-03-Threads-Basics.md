...
И давайте построим "вероятностное" распределение значений: 

![](./img/TasksSpreadInCpuTicks_tp-destribution.png)

На рисунке видно, что очень много значений "уходят вправо". Т.е. есть достаточно много (если всё сложить) значений в зоне очень больших накладных расходов на работу ThreadPool. Увеличим более значимую часть:

![](./img/TasksSpreadInCpuTicks_tp-destribution-zoom.png)

На рисунке становится заметно, что на малых размерах работы накладные расходы ложатся по всему диапазону (бледные графики идут через все три пика). 
...
![](./img/TasksSpreadInCpuTicks_tp-destribution-zoom.png)
![](./img/TasksSpreadInCpuTicks_stp-distribution-zoom.png)
