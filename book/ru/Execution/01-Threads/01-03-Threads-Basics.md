...
Первая 1,000 значений {.description}

![](./img/TasksSpreadInCpuTicks.png) 

Весь диапазон значений {.description}
...
Попробуем драматически увеличить длительность внутреннего цикла до `100_000` итераций. Стоимость такого цикла стала примерно равна 87,000 тактам, что составит более `99%` времени работы (исли считать с издержками). Тогда мы получим, что большинство операций издержек стало занимать около 550 тактов, что практически идеал:

![](./img/TasksSpreadInCpuTicks_good_sorted.png)

Давайте помотрим общую зависимость (.NET 5/.NET 6 (одинаково))

| Итераций цикла | Издержки, тактов | Работа, тактов | Цена единицы работы | Издержки, % | Работа, % |
|----------------|------------------|----------------|---------------------|-------------|-----------|
| 0              | 791 (13 потоков) | 80             | ?                   | 90,81       | 9,19      |
| 5              | 844              | 164            | 201,66              | 83,73       | 16,27     |
| 11             | 797              | 190            | 89,85               | 80,70       | 19,30     |
| 51             | 772              | 751            | 29,88               | 50,68       | 49,32     |
| 101            | 477              | 1839           | 22,94               | 20,61       | 79,39     |
| 501            | 329              | 5715           | 12,07               | 5,45        | 94,55     |
| 1001           | 344              | 9884           | 10,22               | 3,37        | 96,63     |
| 5001           | 365              | 42135          | 8,50                | 0,86        | 99,14     |
| 10001          | 365 (22 потока)  | 81637          | 8,20                | 0,45        | 99,55     |
| 100001         | 474 (24 потока)  | 822035         | 8,23                | 0,06        | 99,94     |
...
Забегая вперёд хочется показать метрики пула потоков собственного производства, речь о котором пойдёт чуть позже:

| Итераций цикла | Издержки, тактов | Работа, тактов | Цена единицы работы | Издержки, % | Работа, % |
|----------------|------------------|----------------|---------------------|-------------|-----------|
| 0              | 154 (1 поток)    | 26             | ?                   | 85,32       | 14,68     |
| 5              | 168              | 32             | 40,17               | 83,86       | 16,14     |
| 11             | 155              | 53             | 19,02               | 74,25       | 25,75     |
| 51             | 136              | 324            | 9,05                | 29,59       | 70,41     |
| 101            | 124              | 592            | 7,10                | 17,41       | 82,59     |
| 501            | 111              | 2769           | 5,75                | 3,86        | 96,14     |
| 1001           | 352 (2 потока)   | 5382           | 5,73                | 6,15        | 93,85     |
| 5001           | 393 (6 потоков)  | 29564          | 5,99                | 1,31        | 98,69     |
| 10001          | 460 (11 потоков) | 68842          | 6,93                | 0,66        | 99,34     |
| 100001         | 612 (24 потока)  | 1424478        | 14,25               | 0,04        | 99,96     |

Из таблицы видно, что издержки упали значительно, работа циклов вообще под вопросом (она стала идти в кэшах процессора) и как результат -- упала цена единицы работы. А всё почему? а всё потому это этот пул создан в т.ч. для решения проблем сверхкороткой работы. В этом случае пул *догадывается* провести всю её на одном потоке вместо параллелизации. Если же его заставить работать на двух: 

| Итераций цикла | Издержки, тактов | Работа, тактов | Цена единицы работы | Издержки, % | Работа, % |
|----------------|------------------|----------------|---------------------|-------------|-----------|
| 0              | 464 (2 потока)   | 32             | ?                   | 93,46       | 6,54      |
| 5              | 427              | 24             | 90,51               | 94,56       | 5,44      |
| 11             | 380              | 28             | 37,16               | 93,08       | 6,92      |
| 51             | 380              | 112            | 9,65                | 77,22       | 22,78     |
| 101            | 365              | 173            | 5,34                | 67,79       | 32,21     |
| 201            | 330              | 311            | 3,19                | 51,49       | 48,51     |
| 501            | 336              | 672            | 2,01                | 33,35       | 66,65     |
| 1001           | 349              | 1243           | 1,59                | 21,94       | 78,06     |
| 5001           | 332 (6 потоков)  | 6028           | 1,27                | 5,22        | 94,78     |
| 10001          | 374 (11 потоков) | 13207          | 1,36                | 2,75        | 97,25     |
| 100001         | 451 (24 потока)  | 220192         | 2,21                | 0,20        | 99,80     |

Метрики испортятся: потоки начнут конкурировать за очередь делегатов.
