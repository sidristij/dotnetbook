...
Поток в терминологии планирования может находиться в нескольких состояниях. Эта машина состояний работает в рамках "Планировщика потоков", который позволяет работать потокам. Когда создаётся поток, он создаётся в статусе Initialized. Далее поток переводится в состояние Готов, ожидание выполнения. Далее планировщик, рассматривает код к исполнению код, он ищет потоки в состоянии Ready. Состояния Deferred Ready и Standby больше относятся к планировщику.

Когда планировщик выбрал поток, переводит его в Standby проверяет приоритеты и переводит его в состояние выполнения Running. Далее поток отрабатывает свой квант времени и планировщик его переводит в состояние Ready или Waiting. 
В состоянии Waiting поток входит в блокировку (блокировки уровня ядра mutex, semaphore, работа с дисковой системой ввода/вывода и т.п.). Когда поток находится в состоянии Waiting, процессор вообще не тратит время на данный поток и он исчезает из планирования. После снятия блокировки поток переводится в состояние Ready и после того, как до него доходит очередь он начинает выполняться. Существует дополнительное состояние Transition, мы его рассматривать не будем. Состояние Terminated - это когда поток завершён.

На каждом ядре есть свой планировщик и много потоков на каждом ядре. Допустим у нас исполняется 50 потоков на ядре. Квант, выделяемый одному потоку около 20 мcек в лучшем случае. Это значит, что на ядре ваше приложение будет получать возможность исполняться в течение 20 млСек раз в секунду. Поэтому если у вас открыто 50 вкладок "Хром" - то надо их все срочно закрыть :)
