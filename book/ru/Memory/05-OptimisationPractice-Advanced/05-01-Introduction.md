...
Траффик объектов приводит, как мы помним, как минимум к GC0. Однако если траффик плотный, то приложение часто просто не успеет освободить ссылки и объекты и вместо очищения памяти в GC0, очищение произойдёт в GC1. В худшей ситуации, с которой я сталкивался, траффик был очень плотным и он был многопоточным. Это приводило к тому, что объекты огромными конгломератами проваливались во 2-е поколение. К чему это приведет? К тому, что GC будет работать дольше. Т.е. ему придется проанализировать более крупный участок памяти для понимания, на какие объекты более нет ссылок. Плюс к этому, если на эти обекты будут ссылаться какие-то старые коллекции или старые объекты (из мира второго поколения), то в игру вступает механизм таблицы карт (подробнее о которой вы можете прочитать в труде Конрада Кососы .NET Memory Management). Что включает еще большие расходы на анализ. Звучит как-то не очень оптимально. Но что же делать? Попахивает-то безнадёгой: сущность ведь всё равно надо выделить. Причем не структуру, а именно класс. И память при этом выделяется и освобождается по полной. Однако, если вспомнить главу "Время жизни сущностей", то мы вспомним, что по сути время жизни любой сущности -- это несколько виртуальное понятие и скорее зависит от восприятия *человека*. Именно человека: ведь по факту никакого выделения памяти не существует. Вместо этого есть разметка физической памяти под какой-то тип и понимание... человеком, что *вот с вот этого* момента объектом можно пользоваться.

[>]: Для меня факт проваливания короткоживущего объекта во второе поколение был не сразу очевиден. Вернее даже так: не сразу очевидны были последствия такого поведения. А последствия таковы, что отношение срабатываний Gen2/Gen0 начинает расти. Т.е. теряется само преимущество существования Gen0-Gen1. При очень плотном траффике всё превращается в непрерывное Gen2.
