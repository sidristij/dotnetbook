...
Здесь вы могли бы задуматься и сказать, что такое поведение возможно только для значимых типов, ибо в противном случае это была бы очередная брешь! И будете правы.
Однако, как бы бессмысленно это не выглядело, компилятор C# посчитает корректным пример использования значимого и ссылочного типа по одному и тому же смещению:
...
{
    [FieldOffset(0)] public uint Value;
    [FieldOffset(0)] public string Reference;
}
```
Лишь во время выполнения, а точнее в момент загрузки данного типа в домен приложения, будет сгенерировано исключение ```TypeLoadException```. Заранее разрушу ваши домыслы о том, что возможно компилятор считает это корректным, из-за возможности объявить  структуру с обощенным (generic) ```T``` параметром вместо ссылочного ```string```, что позволило бы загружать тип структуры, закрытый значимым типом параметра в домен приложения. Это не так, вы все также получите ```TypeLoadException``` во время выполнения.
