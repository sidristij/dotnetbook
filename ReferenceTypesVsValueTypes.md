...
Также интересным фактом является то, что распаковка не является копированием данных из кучи: распаковка является передачей указателя на кучу с проверкой совместимости типов. А вот следующий опкод IL определяет, что будет с этим адресом: возможно, данные будут скопированы в локальную переменную, а может быть - скопированы в стек для вызова метода.
