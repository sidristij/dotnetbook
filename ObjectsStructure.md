...
Диспетчеризация виртуальных методов через заглушки или Virtual Stub Dispatch (VSD) была разработана еще в 2006 году как замена таблицам виртуальных методов в интерфейсах. Основная идея этого подхода состоит в упрощении кодогенерации и последующего упрощения вызова метода. 

### Токены диспетчеризации

Dispatch token или токен диспетчеризации - это структура, содержащая пару значений: идентификатора типа и номера слота. И чтобы облегчить интеграцию с рантаймом реализация подразумевает что порядок номеров слотов будет совпадать с порядком классической VMT. Это означает что вы можете взаимодействовать с VMT, MethodDescs и номерами слотов практически также как и раньше за тем исключением что VMT будет доступна через вызов специального метода вместо того чтобы быть доступной напрямую чтобы скрыть эту абстракцию. 
