...
Давайте для начала рассмотрим код, [приведенный самой компанией Microsoft](https://github.com/dotnet/coreclr/blob/32f0f9721afb584b4a14d69135bea7ddc129f755/src/vm/contractimpl.h). Базовые структуры:
...
// Этот класс призван помочь с быстрым поиском VMT по TypeId и TypeId по VMT
class TypeIDMap
{
protected:
...
DispatchMapEntry или токен диспетчеризации - это структура, содержащая пару значений: идентификатора типа и номера слота. И чтобы облегчить интеграцию с рантаймом реализация подразумевает что порядок номеров слотов будет совпадать с порядком классической VMT. Это означает что вы можете взаимодействовать с VMT, MethodDescs и номерами слотов практически также как и раньше за тем исключением что VMT будет доступна через вызов специального метода вместо того чтобы быть доступной напрямую чтобы скрыть эту абстракцию.
