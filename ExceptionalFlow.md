...
В общем случае мы всегда знаем о тех исключениях, которые произойдут в наших программах потому что мы сами пишем тот код, который работает. Однако, существует множество ситуаций, когда это перестает быть возможным. Например, когда ошибочные ситуации обрабатываются сторонним кодом некоторой библиотеки. Или же когда мы имеем дело с плагинами в нашему приложению, которые загружены в изолированный домен. И как раз в этом случае было бы крайне полезно уметь получать данные о работе изолированного от нас кода. Ведь вполне реальной может быть ситуаций, когда сторонний код перехватывает все без исключения, заглушив из `fault` блоком:
